...
Function Show-zHashcatUsage {
    ...
    Write-Host "`n Usage: zHashcat_v0.2.ps1 <mode> <hashFilename> <client>" -ForegroundColor Yellow
    Write-Host "`n  This is NOT ran with the --username flag" -ForegroundColor Red
    Write-Host "`n Runs all wordlists:" -ForegroundColor Yellow
...
C:\hashcat-6.2.6\hashcat.exe --quiet -m $hashMode -w4 -O $hashFilename D:\Wordlists\Public-Wordlists\*
$UpdateOne=(C:\hashcat-6.2.6\hashcat.exe -m $hashMode -w4 -O $hashFilename D:\Wordlists\Status.txt)  | Select-String "Recovered" 
...
foreach($rule in $rules){
    C:\hashcat-6.2.6\hashcat.exe -m $hashMode -w4 -O $hashFilename $rockYou -r .\rules\$rule --quiet
}
$UpdateTwo=(C:\hashcat-6.2.6\hashcat.exe -m $hashMode -w4 -O $hashFilename D:\Wordlists\Status.txt)  | Select-String "Recovered" 
...
foreach($rule in $rules){
    C:\hashcat-6.2.6\hashcat.exe -m $hashMode -w4 -O $hashFilename $rockTastic -r .\rules\$rule --quiet
}
$UpdateThree=(C:\hashcat-6.2.6\hashcat.exe -m $hashMode -w4 -O $hashFilename D:\Wordlists\Status.txt)  | Select-String "Recovered" 
$Update3=$UpdateThree -replace 'Digests(.*)', '' 
Send-PwdRecoveryUpdate -Title "Hashcat Update for Client: $client" -Text  "Stage 3 - Rocktastic and all the rule sets have been tried, this is the results so far: $Update3"

# Using realuniq
$realUniq="D:\Wordlists\Public-Wordlists\F-realuniq.lst"
Write-Host Now using the wordlist $realUniq
foreach($rule in $rules){
    C:\hashcat-6.2.6\hashcat.exe -m $hashMode -w4 -O $hashFilename $realUniq -r .\rules\$rule --quiet
}
$UpdateFour=(C:\hashcat-6.2.6\hashcat.exe -m $hashMode -w4 -O $hashFilename D:\Wordlists\Status.txt)  | Select-String "Recovered" 
$Update4=$UpdateFour -replace 'Digests(.*)', '' 
Send-PwdRecoveryUpdate -Title "Hashcat Update for Client: $client" -Text  "Stage 4 - Realuniq and all the rule sets have been tried, this is the results so far: $Update4"
...
foreach($rule in $rulesBig){
    C:\hashcat-6.2.6\hashcat.exe -m $hashMode -w4 -O $hashFilename $b0n3z -r .\rules\$rule --quiet
}
$UpdateFive=(C:\hashcat-6.2.6\hashcat.exe -m $hashMode -w4 -O $hashFilename D:\Wordlists\Status.txt)  | Select-String "Recovered" 
$Update5=$UpdateFive -replace 'Digests(.*)', '' 
Send-PwdRecoveryUpdate -Title "Hashcat Update for Client: $client" -Text  "Stage 5 - b0n3z and all 3 rule sets have been tried, this is the results so far: $Update5"
...
foreach($rule in $rulesBig){
    C:\hashcat-6.2.6\hashcat.exe -m $hashMode -w4 -O $hashFilename $weakPass3a -r .\rules\$rule --quiet
}
$UpdateSix=(C:\hashcat-6.2.6\hashcat.exe -m $hashMode -w4 -O $hashFilename D:\Wordlists\Status.txt)  | Select-String "Recovered" 
$Update6=$UpdateSix -replace 'Digests(.*)', '' 
Send-PwdRecoveryUpdate -Title "Hashcat Update for Client: $client" -Text  "Stage 6 - Weakpass_3a and all 3 rule sets have been tried, this is the results so far: $Update6"
...
$UpdateFinal=(C:\hashcat-6.2.6\hashcat.exe -m $hashMode -w4 -O $hashFilename D:\Wordlists\Status.txt)  | Select-String "Recovered" 
